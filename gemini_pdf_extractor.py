...
if len(sys.argv) < 2:
    print("\n📘 使用方式：")
    print("python gemini_pdf_extractor.py <PDF檔案路徑> [每輪處理頁數] [每輪間隔秒數]\n")
    print("範例：")
    print("python gemini_pdf_extractor.py report.pdf 30 10\n")
    sys.exit(0)

PDF_INPUT_FILENAME = sys.argv[1]
...
# === 6️⃣ 檔案檢查與設定 ===
if not os.path.exists(PDF_INPUT_FILENAME):
    print(f"❌ 錯誤：找不到檔案「{PDF_INPUT_FILENAME}」。")
    sys.exit(1)
...
print(f"🟢 每輪間隔秒數： {WAIT_SECONDS}")
print(f"🟢 輸出檔案： {TXT_OUTPUT_FILENAME}")
